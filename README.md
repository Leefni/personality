...
Run API checks through the shared cross-platform entrypoint:

- `BASE_URL="http://localhost/personality" php tests/run_api_checks.php`

This command now runs both:

1. `node tests/frontend_syntax_check.mjs` to validate JavaScript syntax for `assets/app.js` and `assets/js/*.js` before deploy.
2. `php tests/api_v1_endpoints_test.php` to exercise API endpoint behavior.

Optional Unix wrappers are also available:
